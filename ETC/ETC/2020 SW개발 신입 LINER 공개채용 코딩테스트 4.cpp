...
vector<vector<string>> solution(vector<vector<string>> snapshots, vector<vector<string>> transactions) {
	...
	for (int i = 0; i < transactions.size(); i++) {
		transactions[i].erase(unique(transactions[i].begin(), transactions[i].end()), transactions[i].end());
	}
	for (int i = 0; i < transactions.size(); i++) {
...
int main() {
	...
	vector<vector<string>> transactions = { {"1", "SAVE", "ACCOUNT2", "100"},{"2", "WITHDRAW", "ACCOUNT1", "50"},{"4", "SAVE", "ACCOUNT3", "500"} };
